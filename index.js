...
function foreach(iterable, loopStatement, thenStatement) {
  ...
  } else if (loopArguments.length > 2) {
    useAsync = true
  ...
  if (useAsync) {
    ...
    function iterate() {
      switch (modifier) {
        case 'key,next':
          loopStatement(keys[index], next);
          break;
        case 'value,next':
          loopStatement(iterable[keys[index]], next);
          break;
        default:
          loopStatement(keys[index], iterable[keys[index]], next);
          break;
      }
    }
      ...
      if (iterable.hasOwnProperty(key)) {
        switch (modifier) {
          case 'key':
            ...
            break;
          case 'value':
            lastArguments = [iterable[key]]
            loopStatement(iterable[key]);
            break;
          default:
            lastArguments = [key, iterable[key]]
            loopStatement(key, iterable[key]);
            break;
        }
      }
    }
  ...
  let promise = {
    ...
    then: function (statement) {
      if (useAsync) {
        ...
        statement.apply(null, lastArguments);
      }
      return this
    },
